# Komentář k zákonu č. 283/1993 Sb., o státním zastupitelství (ZSZ)

## Struktura
- [00 — Úvod](00-uvod.md) — ústavní postavení (Čl. 80 Ústavy), 4 úrovně, novely
- [01 — Celý zákon (§ 1–46)](01-cely-zakon.md) — všech 13 částí: postavení, pravomoci, soustava, státní zástupci, výkon dozoru, kárná odpovědnost, závěrečná

## Klíčové instituty
- **Čl. 80 Ústavy** — SZ zastupuje veřejnou žalobu
- **Čtyřúrovňová soustava**:
  - Nejvyšší státní zastupitelství (NSZ) — Brno
  - Vrchní státní zastupitelství — Praha, Olomouc
  - Krajská státní zastupitelství — 8 (paritně se soustavou krajských soudů)
  - Okresní (obvodní v Praze) státní zastupitelství
- **NSZ** — jmenovaný vládou; **7leté funkční období** od novely 2024
- **Pravomoci**:
  - Trestní stíhání + dozor nad PČR (§ 174 TŘ)
  - Civilní zastupování státu v určitých řízeních (OSŘ § 35)
  - Účast v řízení o pozůstalosti
  - Mezinárodní právní pomoc
- **Kárná odpovědnost** — kárný senát NSS (od 2002, novela 7/2002)

## Vazby
[Ústava 1/1993](/home/vojtech/Komentar-Ustava-Listina-1-2-1993/) Čl. 80, [TŘ 141/1961](/home/vojtech/Komentar-TR-141-1961/) § 174, [OSŘ 99/1963](/home/vojtech/Komentar-OSR-99-1963/) § 35, [SŘS 150/2002](/home/vojtech/Komentar-SRS-150-2002/) (kárný senát NSS).

## Právní modality

- [PRAVNI-MODALITY.md](PRAVNI-MODALITY.md) — výklad příkazů, zákazů, dovolení, zmocnění, oprávnění a povinností, domněnek a fikcí použitých v předpisu.

## Zásady z místní judikatury

- [ZASADY.md](ZASADY.md) — právní zásady a judikatorní teze extrahované výhradně z místní složky `judikatura/`, bez mazání existujících citací.

<!-- LOCAL-VYKLADOVE-OTAZKY-LINK -->
## Výkladové otázky a odpovědi

- [VYKLADOVE-OTAZKY.md](VYKLADOVE-OTAZKY.md) — předpisově konkrétní kontrolní otázky, odpovědi, místní opora a výslovně označené mezery komentáře.

<!-- LOCAL-HLOUBKOVY-VYKLAD-LINK -->
## Hloubkový výklad a aplikační mapa

- [HLOUBKOVY-VYKLAD.md](HLOUBKOVY-VYKLAD.md) — třináctivrstvá předpisová syntéza, místní důkazní opora, kapitolová matice a autorský protokol úplného právního komentáře.
